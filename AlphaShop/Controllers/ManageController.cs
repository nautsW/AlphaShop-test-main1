    ...
    public class ManageController : Controller
    {
        ...
        public IActionResult AddDrink(DrinkAddVM VM)
        {
            ...
            if (check == null)
            {
                ...
                {
                    PrdId = _context.Products.Count(),
                    CgrId = vM.CategoryID,
                    PrdName = vM.PrdName,
                    PrdPrice = vM.PrdPrice,
                    PrdDesc = vM.PrdDesc,
                    PrdStatus = vM.PrdStatus,
                    PrdImage = (vM.PrdImage == null)? "png-clipart-fizzy-drinks-logo-cocktail-glass-cocktail-glass-logo.png" : filename,
                    PrdVisible = true
                };
            ...
            else
            {
                ...
                check.PrdImage = (vM.PrdImage == null) ? "png-clipart-fizzy-drinks-logo-cocktail-glass-cocktail-glass-logo.png" : filename;
        ...
        public IActionResult Update(DrinkAddVM vm)
        {
            ...
            else
            {
                ...
                check.PrdStatus = vm.PrdStatus;
                check.PrdDesc = vm.PrdDesc;
                if(vm.PrdImage != null) check.PrdImage = filename; 
                _context.Update(check);
                _context.Entry(check).State = EntityState.Modified;
                _context.SaveChanges();
                TempData["msg"] = "<script>alert('Change succesfully');</script>";
